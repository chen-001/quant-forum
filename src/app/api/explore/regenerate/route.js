        ...
        try {
            ...
            if (commentInfo) {
                activityLogQueries.create({
                    ...
                    action: 'exploration_regenerated',
...
async function generateExplorationVariants(commentContent, imageBase64List = []) {
    ...
    try {
        ...
        let jsonContent = codeBlockMatch ? codeBlockMatch[1] : content;

        // 预处理：将三引号"""..."""转换为转义字符串
        jsonContent = preprocessTripleQuotes(jsonContent);
        ...
        try {
            ...
            if (jsonMatch) {
                let extractedJson = preprocessTripleQuotes(jsonMatch[0]);
...
/**
 * 预处理JSON字符串，将三引号"""..."""转换为合法的JSON字符串
 * @param {string} content - 原始内容
 * @returns {string} 处理后的内容
 */
function preprocessTripleQuotes(content) {
    // 匹配三引号包裹的内容："""..."""
    const tripleQuoteRegex = /"""([\s\S]*?)"""/g;

    return content.replace(tripleQuoteRegex, (match, innerContent) => {
        // 将内部内容转义为合法的JSON字符串
        // 1. 转义双引号
        // 2. 将换行符转换为\n
        // 先去除首尾的空白行
